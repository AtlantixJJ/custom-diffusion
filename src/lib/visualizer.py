# python 3.7
# pylint: disable=invalid-name,wrong-import-position,too-many-arguments,no-member,too-many-instance-attributes
"""Utility functions for visualizing results."""
import sys, base64, os, cv2, math, torch
...
from lib.misc import bu, torch2numpy, torch2image, GeneralThread



def plot_heatmap(data, row_labels, col_labels, ax=None, cbar_kw=None, cbarlabel="", **kwargs):
    """
...
def plot_grid(disp):
    fig, axes = plt.subplots(disp.shape[0], disp.shape[1], figsize=(40, 40))
    plt.tight_layout()
    for i in range(disp.shape[0]):
        for j in range(disp.shape[1]):
            axes[i, j].axis("off")
            axes[i, j].imshow(disp[i, j].permute(1, 2, 0))


def draw_box(image, box, d, c):
    """
    Args:
      image: (N, 3, H, W)
      box: (x, y, h, w)
    ...
    for i in range(len(c)):
        image[:, i, x : x + d, y : ye + d] = c[i]
        image[:, i, xe + d : xe + 2 * d, y : ye + 2 * d] = c[i]
        image[:, i, x : xe + d, y : y + d] = c[i]
        image[:, i, x : xe + 2 * d, ye + d : ye + 2 * d] = c[i]
    return image


def visualize_scoremap(image, label, smap, w=None):
    """Visualize the score map of each class.
    Args:
      viz_image: (1, 3, H, W)
      label: (H, W)
      smap: (K, H, W)
    Returns:
      A visualization of (H, W, 3) in numpy.uint8 format
    """
    label_viz = segviz_torch(label.cpu())
    pred_viz = segviz_torch(smap.argmax(0).cpu())
    heatmap = heatmap_torch(smap.cpu())
    if w is not None:
        lens = w.norm(p=2, dim=1)  # (K,)
        lens = lens / lens.max() * smap.shape[1]
        for j in range(w.shape[0]):  # (K, 3, H, W)
            heatmap[j, 0, -int(lens[j]) :, -3:] = 0
            heatmap[j, 1, -int(lens[j]) :, -3:] = 1
            heatmap[j, 2, -int(lens[j]) :, -3:] = 0
    frame = (
        vutils.make_grid(
            torch.cat([image[:1], label_viz[:1], pred_viz[:1], heatmap[:45]]),
            nrow=8,
            padding=2,
            pad_value=0,
        ).permute(1, 2, 0)
    ...
    return torch2numpy(frame).astype("uint8")


def visualize_segmentation(image, pred, size=256):
    pred_viz = segviz_torch(pred.cpu())
    disp = []
    for i in range(image.shape[0]):
        disp.extend([image[i : i + 1], pred_viz[i : i + 1]])
    frame = (
        vutils.make_grid(
            torch.cat(bu(disp, size)), nrow=8, padding=2, pad_value=0
        ).permute(1, 2, 0)
        * 255
    )
    return torch2numpy(frame).astype("uint8")
...
def visualize_combination(viz_image, w, smap, ind1, ind2):
    K, H, W = smap.shape
    pred = smap.argmax(0).cpu()
    pred_viz = segviz_torch(pred)
    target_viz = pred_viz * (pred == ind1).float().view(1, 1, H, W)
    source_viz = pred_viz * (pred == ind2).float().view(1, 1, H, W)
    hmap = heatmap_torch(smap.clamp(-1, 1).cpu())
    lens = w.norm(p=2, dim=1)  # (K,)
    lens = lens / lens.max() * smap.shape[1]
    for j in range(w.shape[0]):  # (K, 3, H, W)
        hmap[j, :, -int(lens[j]) :, -3:] = 0
    disp = [
        viz_image[:1],
        pred_viz[:1],
        hmap[ind1 : ind1 + 1],
    ...
    frame = (
        vutils.make_grid(torch.cat(disp), nrow=2, padding=2, pad_value=0).permute(
            1, 2, 0
        )
        * 255
    )
...
def get_images_SE(G, SE, P, wp, size=256):
    with torch.no_grad():
        if hasattr(G, "synthesis"):
            image, feature = G.synthesis(wp, generate_feature=True)
        else:
            image, feature = G(wp, generate_feature=True)
        seg = SE(feature)
        if P is not None:
            label = P(image, size=size).long()
            return bu(image, size), seg, label
    return bu(image, size), seg, None


def viz_SE(G, SE, P, wp, size=256):
    """Get the images, segmentations, and layer semantics for visualization."""
    images, hmaps = [], []
    layer_vizs, seg_vizs, label_vizs = [], [], []
    for i in range(wp.shape[0]):  # batch
        image, segs, label = get_images_SE(G, SE, P, wp[i : i + 1], size)
        images.append(image)
        hmaps.append(heatmap_torch(bu(segs[-1], size)[0].clamp(-1, 1).cpu()))
        imgs = []
        for seg in segs:
            seg_label = bu(seg, size)[0].argmax(0)
            imgs.append(segviz_torch(seg_label))
        seg_vizs.append(imgs[-1])
        layer_vizs.append(torch.cat(imgs))
        if label is not None:
            label_vizs.append(segviz_torch(label[0]))
    images = bu(torch.cat(images).clamp(-1, 1).cpu() + 1, size) / 2
    layer_vizs = torch.stack(layer_vizs).cpu()
    seg_vizs = torch.cat(seg_vizs).cpu()
    label_vizs = torch.cat(label_vizs).cpu() if label is not None else None
    hmaps = torch.stack(hmaps).cpu()
    if label is None:
        label_vizs = None
    return images, seg_vizs, label_vizs, layer_vizs, hmaps


def color_to_rgbformat():
    s = "["
    for c in high_contrast:
        s += f"'rgb({c[0]}, {c[1]}, {c[2]})', "
    s = s[:-2] + "]"
    print(s)
...
def get_label_color(label_idx):
    return high_contrast[label_idx]


def has_label_color(color):
    return list(color)[:3] in high_contrast


def which_label(color):
    return high_contrast.index(list(color)[:3])
...
def segviz_torch(segs):
    """Pytorch tensor version of segviz_numpy
    [Warning]: This functions breaks backward compatibility.
    Args:
      seg: (N, H, W)
    Returns:
      (N, C, H, W)
    ...
    vizs = [segviz_numpy(s.detach().cpu().numpy()) for s in segs]
    x = torch.from_numpy(np.stack(vizs))  # (N, H, W, 3)
    return x.float().permute(0, 3, 1, 2) / 255.0
...
class FigureAnimator:
    """Animate the frames of a figure."""
    ...
    def __call__(self, fig):
        """Add a frame."""
        self.frames.append(fig2data(fig))
        
    def clean_sync(self):
        """Synchronize to video file."""
        VideoWriter.gen(f"{self.prefix}.mp4", self.frames)


class SimpleVideoRecorder:
    """Record the merging process as a video."""

    def __init__(self, prefix, n_viz, is_norm=True):
        self.n_viz = n_viz
        self.prefix = prefix
        self.thread = None
        self.is_norm = is_norm
        self.frames = {"merge": [], "class": [], "score": []}
        self.args = {"n_viz": n_viz, "prefix": prefix, "is_norm": is_norm}
        self.comb_frames = []
        self.class_frames = []
        self.score_frames = []

    @staticmethod
    def _worker(model, viz_image, feat, label, ind1, ind2, frames, args):
        n_viz, prefix = args["n_viz"], args["prefix"]
        is_norm = args["is_norm"]
        N, H, W = label.shape
        C = feat.shape[-1]
        n_viz = min(n_viz, label.shape[0])
        label_viz = segviz_torch(label[:n_viz].cpu())
        with torch.no_grad():
            smap = []
            viz_feat = feat.view(N, H * W, -1)[:n_viz].view(-1, C)
            seg = model(viz_feat).cpu()  # (NHW, K)
            smap = seg.view(n_viz, H, W, -1).permute(0, 3, 1, 2)
            pred = smap.argmax(1)
            if ind1 >= 0:
                mask = (pred == ind1) | (pred == ind2)
            else:
                mask = pred == ind2
            csmap = smap / smap.abs().max() if is_norm else smap.clamp(-1, 1)
            hmap = heatmap_torch(csmap[0])
            lens = model.weight.norm(p=2, dim=1)  # (K,)
            lens = lens / lens.max().clamp(min=1e-9) * hmap.shape[2]
            for j in range(smap.shape[1]):
                hmap[j, :, -int(lens[j]) :, -3:] = 0
                hmap[j, 1, -int(lens[j]) :, -3:] = 1
            pred_viz = segviz_torch(pred)
            fmask = mask.float().view(n_viz, 1, H, W)
            merge_viz = pred_viz * fmask
            merge_viz += (1 - fmask) * 0.1  # gray background

            # visualization of class merging process
            disp = []
            nrow = 8 if is_norm else 10
            if ind1 >= 0:
                for k in range(n_viz):
                    ...
                    if is_norm:
                        t = (smap[k, ind1] - smap[k, ind2]) * mask[k]
                        shmap = heatmap_torch(t.clamp(-1, 1).unsqueeze(0))
                        disp.append(shmap)
                    else:
                        shmap1 = heatmap_torch(smap[k : k + 1, ind1])
                        shmap2 = heatmap_torch(smap[k : k + 1, ind2])
                        disp.extend([shmap1, shmap2])
                disp = torch.cat(bu(disp, 128))
                frames["merge"].append(
                    torch2image(
                        vutils.make_grid(disp, nrow=nrow, padding=2, pad_value=0),
                        "[0,1]",
                    )
                )
                VideoWriter.gen(f"{prefix}_comb.mp4", frames["merge"])
            else:
                for k in range(n_viz):
                    shmap2 = heatmap_torch(smap[k : k + 1, ind2])
                    shmap1 = torch.zeros_like(shmap2)
                    disp.extend(
                        [
                            viz_image[k : k + 1],
                            pred_viz[k : k + 1],
            ...
            # score map of the first image
            disp = [viz_image[:1], label_viz[:1], pred_viz[:1], hmap[:67]]
            disp = torch.cat(bu(disp, 128))
            frames["score"].append(
                torch2image(
                    vutils.make_grid(
                        disp, nrow=min(10, disp.shape[0]), padding=2, pad_value=0
                    ),
                    "[0,1]",
                )
            )
            VideoWriter.gen(f"{prefix}_score.mp4", frames["score"])

            # class map of visualized images
            disp = []
            for i in range(min(viz_image.shape[0], 15)):
                disp.extend(
                    [viz_image[i : i + 1], label_viz[i : i + 1], pred_viz[i : i + 1]]
                )
            disp = torch.cat(bu(disp, 128))
            frames["class"].append(
                torch2image(
                    vutils.make_grid(disp, nrow=9, padding=2, pad_value=0), "[0,1]"
                )
            )
            VideoWriter.gen(f"{prefix}_class.mp4", frames["class"])

    def __call__(self, *args):
        """Write a frame of merging process."""
        if self.thread is not None:
            self.thread.join()
        self.thread = GeneralThread(
            SimpleVideoRecorder._worker, *args, self.frames, self.args
        )
        self.thread.start()

    def clean_sync(self):
        """Synchronize to the video file."""
        if self.thread is not None:
            self.thread.join()
            self.thread = None
...
class VideoRecorder(object):
    ...
    def __init__(self, prefix, n_viz, is_clamp=True):
        self.n_viz = n_viz
        self.prefix = prefix
        self.thread = None
        self.comb_frames = []
        ...
        with torch.no_grad():
            ...
            if SLSE is not None:
                for i in range(n_viz):
                    f = [f[i : i + 1] for f in feat]
                    smap.append(SLSE(f)[-1].cpu())  # (B, K, H, W)
                smap = torch.cat(smap)
            else:
                viz_feat = feat.view(N, H * W, -1)[:n_viz]
                C = w.shape[1]
                seg = torch.matmul(viz_feat.view(-1, C), w.to(viz_feat).permute(1, 0))
                if b is not None:
                    seg = seg + b.unsqueeze(0)
                seg = seg.cpu()
                smap = seg.view(n_viz, H, W, -1).permute(0, 3, 1, 2)
            pred = smap.argmax(1)  # n_viz, C, H, W

            mask = (pred == ind1) | (pred == ind2)
            if is_clamp:
                csmap = smap.clamp(-1, 1)
            else:
                csmap = smap / smap.abs().max()
            hmap = heatmap_torch(csmap[0])
            w = SLSE.w if SLSE is not None else w
            lens = w.norm(p=2, dim=1)  # (K,)
            lens *= smap.shape[2] / lens.max().clamp(min=1e-9)
            for j in range(smap.shape[1]):
                hmap[j, :, -int(lens[j]) :, -3:] = 0
                hmap[j, 1, -int(lens[j]) :, -3:] = 1
            pred_viz = segviz_torch(pred)
            merge_viz = pred_viz * mask.float().view(n_viz, 1, H, W)

            # visualization of class merging process
            if ind1 >= 0:
                disp = []
                for k in range(n_viz):
                    shmap = heatmap_torch(csmap[k, [ind1, ind2]])
                    disp.extend(
                        [
                            viz_image[k : k + 1],
                            pred_viz[k : k + 1],
                ...
                disp = bu(torch.cat(disp), 128)
                comb_frames.append(
                    torch2image(
                        vutils.make_grid(disp, nrow=10, padding=2, pad_value=0), "[0,1]"
                    )
                )
                VideoWriter.gen(f"{prefix}_comb.mp4", comb_frames)
            ...
            VideoWriter.gen(f"{prefix}_score.mp4", score_frames)

            # class map of visualized images
            if simage is not None:
                if SLSE is not None:
                    f = [f[:1] for f in sfeat]
                    spred = SLSE(f)[-1].argmax(1).cpu()  # (N, C, H, W)
                else:
                    viz_feat = sfeat.view(-1, H * W, C)[:1]
                    seg = torch.matmul(viz_feat.view(-1, C), w.permute(1, 0))
                    spred = seg.view(1, H, W, -1).permute(0, 3, 1, 2).cpu()
                spred_viz = segviz_torch(spred)
                slabel_viz = segviz_torch(slabel[:1])
                disp = [simage[0], slabel_viz[0], spred_viz[0]]
                for i in range(min(viz_image.shape[0], 14)):
                    disp.extend([viz_image[i], label_viz[i], pred_viz[i]])
            else:
                disp = []
                for i in range(min(viz_image.shape[0], 15)):
                    disp.extend([viz_image[i], label_viz[i], pred_viz[i]])
                disp = bu(torch.stack(disp), 128)
            class_frames.append(
                torch2image(
                    vutils.make_grid(disp, nrow=9, padding=2, pad_value=0), "[0,1]"
                )
            )
            VideoWriter.gen(f"{prefix}_class.mp4", class_frames)
...
class VideoWriter(object):
    ...
    def __init__(self, path, frame_height, frame_width, fps=24.0, codec="mp4v"):
    ...
    def write_list(self, frames):
        for frame in frames:
            cH, oH = frame.shape[0], frames[0].shape[0]
            if cH < oH:
                frame = np.concatenate(
                    [frame, np.zeros((oH - cH, *frame.shape[1:]))]
                ).astype("uint8")
            self.write(frame)

    @staticmethod
    def gen(fname, frames):
        VideoWriter(fname, *frames[0].shape[:2]).write_list(frames)
